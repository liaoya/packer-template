...
if grep -s -q -e '^GRUB_CMDLINE_LINUX=""' /etc/default/grub; then
    ...
    [ -f /etc/udev/rules.d/70-peristent-net.rules ] && rm -f /etc/udev/rules.d/70-peristent-net.rules
    sed -i 's/^GRUB_CMDLINE_LINUX=".*/GRUB_CMDLINE_LINUX="net.ifnames=0 biosdevname=0"/' /etc/default/grub
    update-grub

    # Consider disable cloud-init service in future
fi
...
if ! id -u "${SSH_USER}" >/dev/null 2>&1; then
    ...
    getent group "${SSH_GROUP}" || sudo groupadd "${SSH_GROUP}"
    ...
    runuser -l "${SSH_USER}" -c "mkdir ${SSH_USER}_HOME/.ssh"
    runuser -l "${SSH_USER}" -c "chmod 700 ${SSH_USER}_HOME/.ssh"
    echo "${VAGRANT_INSECURE_KEY}" > "${SSH_USER}_HOME/.ssh/authorized_keys"
    chown "${SSH_USER}:$(id -gn ${SSH_USER})" "${SSH_USER}_HOME/.ssh/authorized_keys"
